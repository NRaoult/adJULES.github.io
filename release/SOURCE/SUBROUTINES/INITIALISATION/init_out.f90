...
!-----------------------------------------------------------------------
  if (echo) WRITE(*,"(50('-'),/,a)") 'init_out' !Jupp
...
!------------------------------------------------------------------------
  IF ( dumpFreq/=0 .OR. nout>1 ) THEN
    IF ( LEN_TRIM(outdir) == 0 ) THEN
      ...
      STOP
    ENDIF
!Jupp    INQUIRE (file=outDir, exist=dirExist )
    dirExist = .true. !Jupp
!------------------------------------------------------------------------
...
!------------------------------------------------------------------------
  IF ( nout < 1 ) THEN
    WRITE(*,"(50('#'),/,a,/,50('#'))")'WARNING: init_out: No output has been requested!'
    RETURN
  ENDIF
...
!     Indicate ASCII format - just avoid netCDF!
      ...
      if (echo) write(*,*) 'init_out: reserving scratch on ',outUnit(iout)
