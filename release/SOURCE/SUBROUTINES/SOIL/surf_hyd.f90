...
,CAN_CPY_H(NPNTS,NTILES) &    ! Jupp auxiliary variable
...
DO n=1,ntiles
  DO j=1,tile_pts(n)
  i = tile_index(j,n)
!$TAF STORE can_wcnt = tape_surfhyd, REC = (iloopcount-1)*ntiles*land_pts + (n-1)*land_pts + j
    IF (e_canopy(i,n)  >   0.0)                                   &
      can_wcnt(i,n) =                                             &
                 MAX( can_wcnt(i,n) - e_canopy(i,n)*timestep, 0. )
!                 softmax( can_wcnt(i,n) - e_canopy(i,n)*timestep, 0.,1.0 )  !Luke
  END DO
END DO
...
!   DEPENDS ON: frunoff
    ...
    can_cpy_h(:,N) = can_cpy(:,N)                                   !Jupp
    CALL frunoff (npnts,tile_pts(n),tile_index(:,n),1.,           &
                can_cpy(:,n),can_cpy_h(:,n),infil_tile(:,n),      & !Jupp
