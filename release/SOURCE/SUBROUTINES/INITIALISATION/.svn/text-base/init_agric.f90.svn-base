...
!-------------------------------------------------------------------------------
  ...
  if (echo) WRITE(*,"(50('-'),/,a)") 'init_agric' !Jupp
...
!   single value.
    CALL openFile( 1,.FALSE.,inUnit,'read',fileFormat,fileName,'old','init_agric',ncType )
  ELSE
    if (echo) then !Jupp
...
! Read data.
  CALL readVar2dComp( readT,readZ,fieldNum,stashCodeAgric,irecPrev(useIndex)  &
                     ,nfieldFile,nheaderFile,nheaderT  &
                     ,nheaderField,nlineField,nxIn,nyIn,inUnit  &
                     ,varName  &
                     ,mapInLand(:),(/(i,i=1,land_pts)/),fileFormat,frac_agr(:)  &
                     ,'init_agric','init_agric',ncType )
