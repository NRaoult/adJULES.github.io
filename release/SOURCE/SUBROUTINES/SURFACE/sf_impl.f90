...
USE prognostics, ONLY: nsnow, snowdepth
...
!Jupp EXTERNAL im_sf_pt,sf_evap,sf_melt,screen_tq,sice_htf
!Jupp EXTERNAL timer
...
!write(*,*) 'sf_impl line 675: ', iloopcount
!Jupp
!$TAF STORE fqw_ice,fqw_tile,ftl_ice,ftl_tile, &
!$TAF       snow_tile = tape_n, REC = iloopcount
!Jupp
...
L_NEG_TSTAR = .true.       !Jupp otherwise undefined
IF (l_neg_tstar) THEN
  DO l=1,land_pts
    IF (t_soil(l,1) < 0) THEN
      error = 1
      WRITE(6,*) '*** ERROR DETECTED BY ROUTINE BDY_LAYR in sf_impl***'
...
!Jupp
!write(*,*) 'sf_impl line 756: ', iloopcount
!$TAF STORE ecan_tile,ei_tile,elake_tile,esoil_tile,tstar_tile =  &
...
DO n=1,ntiles
!Luke
!$TAF STORE ei_tile,tstar_tile=tape_sfimpl1, REC=(iloopcount-1)*ntiles+n
!Luke
...
! DEPENDS ON: sf_melt
  CALL sf_melt (                                                  &
   ...
   tstar_tile(:,n),snow_tile(:,n),snowdepth(:,n),                 &
   melt_tile(:,n)                                                 &
   )

!Jupp
!write(*,*) 'sf_impl line 781: ', n+(iloopcount-1)*ntiles
!$TAF STORE ei_tile,ftl_tile,melt_tile,tstar_tile = tape_n_nt,       &
!$TAF       REC = n+(iloopcount-1)*ntiles
!Jupp
...
DO n=1,nice
!Luke
!$TAF STORE ei_sice,tstar_sic=tape_sfimpl, REC=(iloopcount-1)*nice+n
!Luke
...
! DEPENDS ON: sf_melt
  CALL sf_melt (                                                  &
   ...
   ei_sice,fqw_1,ftl_1,fqw_ice,ftl_ice,                           &
...
DO j=1,rows
  DO i=1,row_length
    ...
    IF (flandg(i,j) < 1.0 .AND. ice_fract(i,j) > 0.0) THEN
      DO n=1,nice
        tstar_sice(i,j) = (ice_fract_ncat(i,j,n)/ice_fract(i,j))  &
                        * tstar_sic(i,j,n)
      END DO
...
!Jupp
!write(*,*) 'sf_impl line 939: ', iloopcount
!$TAF STORE tstar_ssi = tape_n, REC = iloopcount
!Jupp
...
!Jupp
!write(*,*) 'sf_impl line 1006: ', iloopcount
!$TAF STORE surf_ht_flux_sice_ncat = tape_n, REC = iloopcount
!Jupp

olr(1,1) = 1.           !Jupp otherwise undefined
DO j=1,rows
  DO i=1,row_length
