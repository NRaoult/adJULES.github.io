!Jupp
!$TAF MODULE initial_mod SUBROUTINE headerSearch INPUT  = 1, 2, 3, 4, 5
!$TAF MODULE initial_mod SUBROUTINE headerSearch OUTPUT =                6, 7
!$TAF MODULE initial_mod SUBROUTINE headerSearch ACTIVE = 
!$TAF MODULE initial_mod SUBROUTINE headerSearch DEPEND = 
! since dowrite==.true. in active part of model:
!$TAF MODULE initial_mod SUBROUTINE dump_io INPUT  = 1, 2, 3
!$TAF MODULE initial_mod SUBROUTINE dump_io OUTPUT = 
!$TAF MODULE initial_mod SUBROUTINE dump_io ACTIVE = 
!$TAF MODULE initial_mod SUBROUTINE dump_io DEPEND = 
!Jupp
...
! Scalar parameters.
!-------------------------------------------------------------------------------
  INTEGER, PARAMETER :: nvarMax = 30   !  the number of all possible variables
!                      that can be used to
...
!  imported scalars with intent(in)
     dim_cs1,land_pts,nsmax,ntiles,sm_levels,soil_pts  &
...
!  imported arrays with intent(in)
  ...
  USE switches, ONLY :  &
!  imported scalars with intent(in)
     can_model,l_phenol,l_spec_albedo,l_top,l_triffid,l_veg_compete,route,routeOnly
...
!                          F means the frozen and unfrozen fractions (sthf and
...
!   We only use need to initialise these from initial conditions when
!   competing vegetation is on - otherwise they get initialised in
!   init_veg_pft
!-------------------------------------------------------------------------------
!   Canopy height.
...
!-------------------------------------------------------------------------------
 !        write(*,*) 'Jupp: warning for readvar3dcomp'
	 
            CALL readVar3dComp( readT,varFlag(ivar),varStash(ivar),.FALSE.  &
                               ,irecPrev(useIndex),nfieldFile  &
                               ,nheaderFile,nheaderT,nheaderField,nlineField  &
                               ,nxIn,nyIn,varNzRead(ivar),zrevFlag  &
                               ,inUnit,varNameSDF(ivar)  &
                               ,mapInLand(:),(/(i,i=1,varSize(ivar,1))/) &
                               ,fileFormat  &
                               ,workSpace(1:varSize(ivar,1),1:varNzRead(ivar))  &
                               ,'init_ic','init_ic',ncType )
          END SELECT  !  var Type
      ENDIF  !  inUnit etc
    ...
    IF ( l_triffid ) THEN
      DO j=1,soil_pts
        ...
        IF ( ANY( frac(i,:)<frac_min ) ) THEN
          WRITE(*,"(10('*'),a,i6,a,10('*'))")' WARNING: point #',i,' frac<frac_min '
          WRITE(*,*)'Resetting to frac_min=',frac_min
...
!  imported arrays with intent(in)
  ...
  USE trifctl, ONLY :   &
!  imported arrays with intent(in)
     g_leaf_acc,g_leaf_phen_acc,npp_ft_acc,resp_s_acc,resp_w_ft_acc
...
!-------------------------------------------------------------------------------
  ...
  IF ( summary ) THEN
    ...
    IF ( tileFracMask ) THEN
      WRITE(*,"(a,es9.1,a,/)")'Only locations with frac>',fracMin,' are considered.'
    ELSE
...
!    written by the code further below.
    nheaderFileOut = 11   &!  number of header lines at start of an ASCII dump file
...
!-------------------------------------------------------------------------------
      ...
      WRITE(header,"(i8,i6,tr1,a,tr8,a,tr2,a)") date,time,TRIM(runID)  & !Jupp
...
!-------------------------------------------------------------------------------
    CALL check_dump_dims( unit,nheaderFieldOut,nheaderFileOut  &
            ...
            ,haveTile,haveType,dumpName,fileFormat )
...
!          write(*,*) 'Jupp: warning: readvar3dcomp'
...
! Close file.
  CALL closeFile( unit,fileFormat )
...
!                                               at top of file
  LOGICAL, INTENT(in) :: haveSCpool,haveLand,havePFT  &
            ,haveSnow,haveRoute,haveSoil  &
            ,haveTile,haveType   !   Flags indicating what "types" of
...
! Local scalar variables.
!------------------------------------------------------------------------------
  INTEGER, PARAMETER :: ndimMax = 9  !  number of possible dimensions
...
!-------------------------------------------------------------------------------
  ...
  INTEGER :: dim_cs1In,land_ptsIn,npftIn,nsmaxIn,ntilesIn  &
         ,ntypeIn,nxRouteIn,nyRouteIn,sm_levelsIn  ! dimension sizes as
