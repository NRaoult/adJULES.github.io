...
!     Space is allocated only for variables that are to be used in determining spin up.
  ...
  LOGICAL :: spinTolPercent(nspinVarMax)=.false.  !Jupp explicit initialisation  T if spinTol is a percentage change.
!                                            F if spinTol is to be used as given (not a %)
  LOGICAL :: spinVar(nspinVarMax)=.false.         !Jupp explicit initialisation  T if a variable is used to determine whether
!                                             model has spun-up
