...
!-------------------------------------------------------------------------------
  ...
  INTEGER, PARAMETER ::   &!  scalar parameters
     & nUnitMax=100    ! Jupp maximum unit used to connect to a file
...
! Local scalar variables.
  INTEGER :: ierr   !  error flag
   ...
   LOGICAL ::  &          !Jupp  local scalars
     existArg,openedArg   !Jupp  arguments for inquire.
...
!     FOR NOW, assuming we want to write a 'default' real value!
      ...
      inquire (unit=unit,iostat=ivar)                   ! Jupp
      if (echo) print*, 'unit, iostat = ',unit, ivar    ! Jupp
      inquire (file=filename,opened=lvar)               ! Jupp
      if (echo)   print*, 'filename, opened = ',lvar    ! Jupp
      inquire (unit=unit,opened=lvar)                   ! Jupp
      if (echo)   print*, 'unit, opened = ',unit, lvar  ! Jupp
      if (lvar) then                                    ! Jupp
         close(unit)                                    ! Jupp
         if (echo)   print*, 'closing ', unit           ! Jupp
      endif                                             ! Jupp
      inquire (unit=unit,opened=lvar)                   ! Jupp
      if (echo)   print*, 'unit, opened = ',unit, lvar  ! Jupp
