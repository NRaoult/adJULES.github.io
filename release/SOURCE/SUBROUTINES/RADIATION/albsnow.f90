...
!Jupp
  DO j=1,tile_pts(n)
    ...
    IF ( snowdepth(l,n)  >   0. ) THEN
      ...
      IF ( sigma  >   1. ) THEN
!$TAF STORE alb_snow = tape_as, REC = j + (n-1)*land_pts + (iloopcount-1)*land_pts*ntiles
        alb_snow(l,n,1) = 0.07 +                                  &
                          0.5*(alb_snow(l,n,1) - 0.07)/(sigma**0.46)
        alb_snow(l,n,2) = 0.07 +                                  &
                          0.5*(alb_snow(l,n,2) - 0.07)/(sigma**0.46)
      ELSE
            if (sigma .gt. 0.) then                                 !Jupp ensures differentiability
!$TAF STORE alb_snow = tape_as, REC = j + (n-1)*land_pts + (iloopcount-1)*land_pts*ntiles
            alb_snow(L,N,1) = alb_snow(L,N,1) -  &                  !Jupp   
                         0.5*(alb_snow(L,N,1) - 0.07)*(sigma**0.6)  !Jupp
            alb_snow(L,N,2) = alb_snow(L,N,2) -  &                  !Jupp                             
                         0.5*(alb_snow(L,N,2) - 0.07)*(sigma**0.6)  !Jupp   
            else                                                    !Jupp
!$TAF STORE alb_snow = tape_as, REC = j + (n-1)*land_pts + (iloopcount-1)*land_pts*ntiles
            alb_snow(L,N,1) = alb_snow(L,N,1)                       !Jupp
            alb_snow(L,N,2) = alb_snow(L,N,2)                       !Jupp	    
	    end if                                                  !Jupp ensures differentaibility
      END IF
    END IF
  END DO
END DO
...
!$TAF STORE alb_snow = tape_as, REC = j + (n-1)*land_pts + (iloopcount-1)*land_pts*ntiles
      ...
      ELSE
	    if (sigma .gt. 0) then                                         !Jupp
!$TAF STORE alb_snow = tape_as, REC = j + (n-1)*land_pts + (iloopcount-1)*land_pts*ntiles
            alb_snow(L,N,3) = alb_snow(L,N,3) -  &                         !Jupp
                              0.5*(alb_snow(L,N,3) - 0.06)*(sigma**0.7)    !Jupp
            alb_snow(L,N,4) = alb_snow(L,N,4) -  &                         !Jupp
                              0.5*(alb_snow(L,N,4) - 0.06)*(sigma**0.7)    !Jupp
            else                                                           !Jupp
!$TAF STORE alb_snow = tape_as, REC = j + (n-1)*land_pts + (iloopcount-1)*land_pts*ntiles
            alb_snow(L,N,3) = alb_snow(L,N,3)                              !Jupp
            alb_snow(L,N,4) = alb_snow(L,N,4)	                           !Jupp
