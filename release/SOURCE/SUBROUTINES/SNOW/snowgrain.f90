...
DO k=1,tile_pts
!Luke
!$TAF STORE rgrain,rgrainl=tape_snow, REC=(iloopcount-1)*tile_pts+k
!Luke
  i = tile_index(k)
!-----------------------------------------------------------------------
...
!      rgrain(i) = softMIN( rmax, rgrain(i),1.0 )  !Luke
!      rgrain(i) = softMAX( r0, rgrain(i),1.0 )    !Luke
    ELSE
...
!-----------------------------------------------------------------------
    DO n=1,nsnow(i)
!Luke
!$TAF STORE rgrainl=tape_snowgrain, REC=(iloopcount-1)*tile_pts*nsnow(i)+(k-1)*nsnow(i)+n
!Luke
      IF ( sice(i,n) > 0 ) THEN
        rate = snow_ggr(1)
...
!        rgrainl(i,n) = softMIN( rmax, rgrainl(i,n),1.0 ) !Luke
!        rgrainl(i,n) = softMAX( r0, rgrainl(i,n),1.0 )   !Luke
      END IF
    END DO
