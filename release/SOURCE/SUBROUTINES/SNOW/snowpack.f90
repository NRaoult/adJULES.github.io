...
USE fomod, only: iloopcount, softmin, softmax !Luke
IMPLICIT NONE
...
DO k=1,tile_pts
!Luke
!$TAF STORE a,asnow,b,c,ds,dt,melt_tile,r,sice,sliq,snow_soil_htf,snowdepth,snowmass,tsnow,tsoil1 &
!$TAF =tape_snow, REC=(iloopcount-1)*tile_pts+k
!Luke
  i = tile_index(k)
...
!      submelt = softMIN( snowmass(i) / timestep,                      &
!                     (tsoil1(i) - tm) / (lf * asoil * timestep),1.0 ) !Luke
...
!    tsnow0(i) = softMIN( tsoil1(i), tm, 1.0) !Luke
  ELSE
...
!$TAF INCOMPLETE a,asnow,b,c,dt,r,snow_soil_htf
...
!      dsice = softMAX( ei_tile(i), 0., 1.0 ) * timestep !Luke
      IF ( dsice > 0.0 ) THEN
        DO n=1,nsnow(i)
!Luke
!$TAF STORE ds,dsice,sice=tape_snowpack, REC=(iloopcount-1)*tile_pts*nsnow(i)+(k-1)*nsnow(i)+n
!Luke
...
! Layer sublimates partially
            ds(i,n) = (1.0 - dsice/sice(i,n))*ds(i,n)
            sice(i,n) = sice(i,n) - dsice
!            EXIT    !   sublimation exhausted !!!Luke commented for adjoint purposes
          END IF
        END DO
      END IF  !  DSICE>0
    END IF  !  CANSNOWTILE
...
!-----------------------------------------------------------------------
    ...
    DO n=1,nsnow(i)
!Luke
!$TAF STORE sliq,win=tape_snowpack, REC=(iloopcount-1)*tile_pts*nsnow(i)+(k-1)*nsnow(i)+n
!Luke
...
!        dsice = softMIN(sliq(i,n), coldsnow / lf, 1.0) !Luke
        sliq(i,n) = sliq(i,n) - dsice
        sice(i,n) = sice(i,n) + dsice
...
!      sice0(i) = snowfall(i) - softMIN(ei_tile(i), 0., 1.0) * timestep !Luke
    tsnow0(i) = tsnow(i,1)
    rho0(i) = rho_snow_const
