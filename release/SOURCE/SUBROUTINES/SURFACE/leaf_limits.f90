...
DO j=1,open_pts
!Jupp
!$TAF INCOMPLETE vcmax,tdegc,qtenf,denom,vcm,rd
!Jupp
...
IF (c3(ft) == 1) THEN
  ...
  DO j=1,open_pts
!Jupp
!$TAF INCOMPLETE wcarb,wlite,wexpt
!Jupp
    l = veg_index(open_index(j))
! The numbers
...
!    wlite(l) = softmax(wlite(l), TINY(1.0e0),1.0) !Luke
    wlite(l) = max(wlite(l), sqrt(sqrt(TINY(1.0e0)))) ! Jupp   NOTE needed one square root to prevent underflow in adjoint code
    wcarb(l) = max(wcarb(l), sqrt(sqrt(TINY(1.0e0)))) ! Jupp        needed second square root to prevent underflow in hessian code
    wexpt(l) = max(wexpt(l), sqrt(sqrt(TINY(1.0e0)))) ! Jupp
  END DO
...
!    wlite(l) = softmax(wlite(l), TINY(1.0e0),1.0) !Luke
    wlite(l) = max(wlite(l), sqrt(sqrt(TINY(1.0e0)))) ! Jupp
    wcarb(l) = max(wcarb(l), sqrt(sqrt(TINY(1.0e0)))) ! Jupp
    wexpt(l) = max(wexpt(l), sqrt(sqrt(TINY(1.0e0)))) ! Jupp
