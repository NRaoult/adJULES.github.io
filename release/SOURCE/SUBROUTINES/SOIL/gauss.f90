...
use fomod, only : iloopcount,softmax,softmin !Luke
IMPLICIT NONE
...
real smk ! Jupp local softmax parameter
...
!  x(i,1)=MIN(x(i,1),xmax(i,1))
  ...
  smk = 50 / (1.0 + abs( xmax(i,1)-xmin(i,1) ) )

  x(i,1)=softmax(x(i,1),xmin(i,1),smk ) !Luke, modified by Jupp to prevent overflow in softmax

!write(*,*)'gauss line 153',x(i,n),xmin(i,n)

  x(i,1)=softmin(x(i,1),xmax(i,1),smk ) !Luke, modified by Jupp to prevent overflow in softmax

!write(*,*)'gauss line 155',x(i,n),xmax(i,n)
END DO
...
!    x(i,n)=MIN(x(i,n),xmax(i,n))
  ...
  smk = 50 / (1.0 + abs( xmax(i,n)-xmin(i,n) ) )

  x(i,n)=softmax(x(i,n),xmin(i,n),smk ) !Luke, modified by Jupp to prevent overflow in softmax
!write(*,*)'gauss line 187',x(i,n),xmin(i,n)
  x(i,n)=softmin(x(i,n),xmax(i,n),smk) !Luke, modified by Jupp to prevent overflow in softmax
!write(*,*)'gauss line 189',x(i,n),xmax(i,n)
  END DO
