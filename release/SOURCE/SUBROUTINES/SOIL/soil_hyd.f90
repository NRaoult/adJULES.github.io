...
!Jupp EXTERNAL hyd_con_ic,darcy_ic,gauss,calc_zw
EXTERNAL gauss!,calc_zw !Jupp
...
!Jupp
!$TAF incomplete DWFLUX_DSTHU1,DWFLUX_DSTHU2
!Jupp
...
IF(l_top)THEN
!Jupp
!$TAF STORE w_flux = tape_n, REC = iloopcount
!$TAF LOOP = PARALLEL
!Jupp
   DO j=1,soil_pts
      i=soil_index(j)
...
!Jupp
!$TAF STORE smcl = tape_n, REC = iloopcount
!$TAF LOOP = PARALLEL
!Jupp
...
!-----------------------------------------------------------------------
  DO n=1,nshyd
    DO j=1,soil_pts
!$TAF STORE w_flux = tape_soilhyd, REC = (iloopcount-1)*nshyd*soil_pts + (n-1)*soil_pts + j
      i=soil_index(j)
...
! DEPENDS ON: gauss
!Jupp
!$TAF STORE dsthu = tape_n, REC = iloopcount
!Jupp
...
!-----------------------------------------------------------------------
!$TAF STORE w_flux,dsmcl,smclsatzw,smclzw = tape_soilhyd, REC = (iloopcount-1)*soil_pts*nshyd + (j-1)*nshyd
...
!-----------------------------------------------------------------------
   DO n=nshyd,1,-1
      DO j=1,soil_pts
         i=soil_index(j)
!$TAF STORE dsmcl = tape_soilhyd1, REC = (iloopcount-1)*soil_pts*nshyd + (j-1)*nshyd
...
DO n=1,nshyd
  DO j=1,soil_pts
!$TAF STORE dsmcl,smclsat,smclu,soil_index = tape_soilhyd, REC = (iloopcount-1)*nshyd*soil_pts + (n-1)*soil_pts + j
    i=soil_index(j)
...
!-----------------------------------------------------------------------
  ...
  DO j=1,soil_pts
    i=soil_index(j)
!$TAF STORE nshyd,qbase_l,smclsatzw,smclzw,soil_index,timestep,w_flux = tape_n_soil, REC = (iloopcount-1)*soil_pts+j
