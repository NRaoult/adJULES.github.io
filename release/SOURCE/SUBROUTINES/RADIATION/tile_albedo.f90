...
SUBROUTINE tile_albedo (                                          &
 ...
 cosz,frac,lai_in,rgrain,snowdepth,soot,tstar_tile,               &
...
use ancil_info, only : land_pts    !Jupp
...
,snowdepth(land_field,ntiles)                                     &
                             ! Depth of lying snow on tiles (m)
,soot(p_field)                                                    &
...
!    lai(l,n) = softmax( lai_in(l,n), 0.5 ,100.0) !Luke
  END DO
END DO

!$TAF INCOMPLETE alb_type
...
!$TAF STORE alb_snow = tape_n, REC = iloopcount 
...
! Adjust surface type albedos for snow cover
  DO l=1,land_field
    snowd(l) = snowdepth(l,1)
    z0(l) = z0_tile(l,1)
  END DO

!$TAF LOOP = PARALLEL
  DO n=1,ntype
!$TAF STORE snowd,tstar,z0 = tape_ta, REC = n + (iloopcount-1)*ntype
...
!$TAF STORE alb_type = tape_ta1, REC = band + 4 * (j-1) + 4*land_pts*(n-1) + 4*land_pts*ntype*(iloopcount-1)
...
!$TAF LOOP = PARALLEL
      DO j=1,tile_pts(n)
!$TAF STORE snowd,tstar = tape_ta2, REC = j + land_pts*(n-1) + land_pts*ntype*(iloopcount-1)
        l = tile_index(j,n)
...
!Jupp
!$TAF STORE snowd,tstar,z0 = tape_n_ta, REC = iloopcount
!Jupp
!----------------------------------------------------------------------
