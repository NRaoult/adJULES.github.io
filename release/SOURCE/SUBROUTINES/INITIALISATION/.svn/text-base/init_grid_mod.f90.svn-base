...
!------------------------------------------------------------------------------------------
  ...
  if (echo) WRITE(*,"(50('-'),/,a)") 'init_grid' !Jupp
...
!-------------------------------------------------------------------------------
  ...
  IF ( readPoints .AND. landOnly .and. echo ) THEN !Jupp
...
!       Locate the information in run control file.
        CALL findTag( jinUnit,'init_grid',tagAscBin,preInit=.TRUE. )
...
!       Locate the information in run control file.
      ...
      CASE default
        if (echo) WRITE(*,*) & !Jupp
	'ERROR: init_grid: no code for fileFormat=',TRIM(fileFormatLand) !Jupp
        STOP
    END SELECT
...
! Ensure we have intervals > 0.
  ...
  IF ( regDlat<EPSILON(regDlat) .OR. regDlon<EPSILON(regDlon) ) THEN
    if (echo) WRITE(*,*) & !Jupp
    'ERROR: init_grid: regDlat or regDlon is too small (effectively zero).'!Jupp
    STOP
...
!         Locate the information in run control file.
        ...
        CASE default
          if (echo) WRITE(*,*) &
	  'ERROR: init_grid: no code for fileFormat=',TRIM(fileFormatLL) !Jupp
          STOP
      END SELECT
...
!     Read and process land fraction.
      ...
      CALL init_grid_land( fieldLand,nheaderFieldLand,nheaderFileLand  &
               ,nx,ny,landOnly,readFileLand  &
               ,fileFormatLand,fileNameLand,varNameLand )

!-------------------------------------------------------------------------------
!     Get latitude and longitude of each gridpoint.
      CALL init_grid_latlon( fieldLat,fieldLon,nheaderFieldLL,nheaderFileLL  &
...
!   Allocate work space.
    ...
    IF ( ierr /= 0 ) THEN
      if (echo) WRITE(*,*) & !Jupp
      'ERROR: init_grid: error allocating usePoint. ierr=',ierr !Jupp
      STOP
    ENDIF
...
!   Deallocate work space.
    ...
    IF ( ierr /= 0 ) WRITE(*,*)'WARNING: init_grid: error deallocating usePoint. stat=',ierr
...
!-------------------------------------------------------------------------------
  IF ( echo ) THEN
    ...
    WRITE(*,"(a,(10i7))") 'land_index=',land_index(:)
...
! Read land fraction and set up land points.
  ...
  SUBROUTINE init_grid_land( fieldLand,nheaderFieldLand,nheaderFileLand  &
             ,nx,ny,landOnly,readFileLand  &
             ,fileFormatLand,fileNameLand,varNameLand,usePoint )
...
!   Use first arg to openFile to set recl (for unformatted file) to be enough for a single value.
    CALL openFile( 1,.FALSE.,inUnit,'read',fileFormatLand,fileNameLand,'old','init_grid',ncType )
  ELSE
    if (echo) WRITE(*,*)'Reading land fraction from the run control file.'
    if (echo) WRITE(*,*)'Data must be the first field encountered.'
    inUnit = jinUnit
!   Locate the start of the data in the run control file.
...
! Read land fraction.
!-------------------------------------------------------------------------------
  CALL readVar2d( readT,readZ,fieldLand,stashLand,irecPrev(useIndex),nfieldFile  &
                 ,nheaderFileLand,nheaderT,nheaderFieldLand  &
                 ,nlineField,nxIn,nyIn,inUnit,varNameLand  &
                 ,mapIn(:),(/ (i,i=1,npoints) /),fileFormatLand  &
                 ,flandg,byteSwap,'init_grid_land','init_grid',ncType )
...
! If necessary, reject points outside given lat/lon range.
  ...
  SUBROUTINE init_grid_latlon( fieldLat,fieldLon,nheaderFieldLL,nheaderFileLL  &
             ,npointsList,nx,ny,latIn,lonIn,llFlag  &
             ,readFileLL,regLatLonIn,subAreaLatLon  &
             ,fileFormatLL,fileNameLL,varNameLat,varNameLon  &
...
!     Use first arg to openFile to set recl (for unformatted file) to be enough for a single value.
      CALL openFile( 1,.FALSE.,inUnit,'read',fileFormatLL,fileNameLL,'old','init_grid',ncType )

    ELSE

      if (echo) WRITE(*,*)'Reading lat/lon from the run control file.'
      if (echo) WRITE(*,*)'These must be the first two fields, in order lat then lon.'
      inUnit = jinUnit
!     Locate the start of the data in the run control file.
...
!   Read data.
!-------------------------------------------------------------------------------
    CALL readVar2d( readT,readZ,fieldLat,stashLat,irecPrev(UseIndex),nfieldFile  &
                   ,nheaderFileLL,nheaderT,nheaderFieldLL  &
                   ,nlineField,nxIn,nyIn,inUnit,varNameLat  &
                   ,mapIn(:),(/(i,i=1,npoints)/),fileFormatLL  &
                   ,latitude,byteSwap,'init_grid_latlon','init_grid',ncType )
    CALL readVar2d( readT,readZ,fieldLon,stashLon,irecPrev(useIndex),nfieldFile  &
                   ,nheaderFileLL,nheaderT,nheaderFieldLL  &
                   ,nlineField,nxIn,nyIn,inUnit,varNameLon  &
                   ,mapIn(:),(/(i,i=1,npoints)/),fileFormatLL  &
                   ,longitude,byteSwap,'init_grid_latlon','init_grid',ncType )
!-------------------------------------------------------------------------------
