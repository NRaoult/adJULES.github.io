...
!-------------------------------------------------------------------------------
  if (echo) WRITE(*,"(50('-'),/,a)") 'init_veg_pft' !Jupp
...
!    write(*,"(a,10(tr1,a))") 'pftName=',(/ (trim(pftName(i)), i=1,npft ) /)  !  not working!
    ...
    WRITE(*,*) 'c3=',c3(:)
    WRITE(*,*) 'canht_ft=',canht_ft(1,:)
    WRITE(*,*) 'lai=',lai(1,:)
    WRITE(*,*) 'catch0=',catch0(:)
    WRITE(*,*) 'dcatch_dlai=',dcatch_dlai(:)
    WRITE(*,*) 'dz0v_dh=',dz0v_dh(:)
...
!-------------------------------------------------------------------------------
  ...
  if (echo) WRITE(*,"(50('-'),/,a)") 'init_veg_vary' !Jupp
...
! Allocate space for some of the veg variables.
  CALL allocate_arrays( 'init_veg_vary 1' ) !Luke to avoid unallocated arrays
...
!     Locate the information in run control file.
    ...
    CASE default
      WRITE(*,*)'ERROR: init_frac: no code for vegFormat=',TRIM(vegFormat)
      STOP
...
!   Read details from another file.
    ...
    ENDDO
    CALL closeFile( iunit,formatAsc )
  ELSE
...
!Luke - this is included to avoid unallocated arrays in adjoint
...
!     unlikely occurence.
      if (echo) WRITE(*,*)'Day of month on which monthly veg data are valid on/timestamped=' &
                 ,vegDataDay
    ENDIF
...
!   Get latest time required.
    ...
    if (echo) PRINT*, 'nvegDataTime=',nvegDataTime
...
!-------------------------------------------------------------------------------
  ...
  CALL allocate_arrays( 'init_veg_vary 2' ) !Luke to avoid unallocated arrays
...
!-------------------------------------------------------------------------------
  IF ( echo ) THEN
    ...
    DO ivar=1,nvegVar
      ...
      ELSE
        line = TRIM(line) // ' is called ' // TRIM(vegVarNameFile(ivar)) // ' in input'
      ENDIF
...
!   Close files and deallocate.
    ...
    ENDDO
    ierrSum = 0
!Luke removed section due to conflict with required variables for adjoint
