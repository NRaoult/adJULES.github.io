...
! Update driving data. This may involve reading data from file.
  ...
  SUBROUTINE drive_update( tstep,next,istepArg )
...
!  imported arrays with intent(inout)
  ...
  USE drive_io_vars, ONLY : drivedatain !Jupp: for tapes

  use fomod, only : iloopcount !Jupp: for tapes
...
!-------------------------------------------------------------------------------
!Jupp  INTEGER, INTENT(in), OPTIONAL :: istepArg  !  current value of the time index
  INTEGER, INTENT(in) :: istepArg  ! Jupp  current value of the time index
...
!-------------------------------------------------------------------------------
  INTEGER ::  &
    i,istep,j,k         &! loop counters/work
...
!--------------------------------------------------------------------------------
  ...
  if (echo) write(*,*) 'entering drive_update with iloopcount = ',iloopcount
  if (echo) write(*,*) 'and arguments',tstep,next,istepArg
! At present this code cannot deal with "special" periods (e.g. monthly data).
! If these are required, veg_update might provide some useful help as to how to do.
...
!    print*,'call find_file in update_mod:',driveTemplateDate
...
!         driveData(driveVarPos(iposPrecipLR),1,1,1),driveData(driveVarPos(iposPrecipTS),1,1,1)
  ...
  ENDIF  !  driveDataStep
!Jupp
 if (echo) write(*,*) 'update_mod 258 :',iloopcount+1
!$TAF STORE drivedata,drivedatastep = tape_n1, REC = iloopcount + 1
!Jupp
...
! Find out what time level of driveData to use.
  istep = driveDataStep
!Jupp  IF ( PRESENT(istepArg) ) istep=istepArg
  IF ( istepArg>0 ) istep=istepArg !Jupp
...
!Jupp   IF ( PRESENT(istepArg) ) THEN
    IF ( istepArg>0 ) then !Jupp
...
!       Hold net fluxes in downward flux variables.
      ...
      ENDIF
! Jupp hack      sw_down(:,:) = MAX( sw_down, 0.0 )   ! hacked this check out to make the adjoint work
    ENDIF
...
!     Close any file that is open on this unit, and open next file.
      CALL openFile( 1,.TRUE.,driveUnit(i),'read',driveFormat,fileName,'old'  &
                    ,'drive_read',ncTypeDrive )
...
!     but need to keep index within bounds.
      ...
      CALL readVar2d( driveFileStep,readZ,driveVarFlag(ipos)  &
                     ,driveVarStash(ipos),irecPrev(useIndex)  &
                     ,nfieldDriveFile,ndriveHeaderFile,ndriveHeaderTime  &
                     ,ndriveHeaderField,0,nxIn,nyIn,unit  &
                     ,driveVarNameSDF(ipos)  &
                     ,mapIn(:),(/(i,i=1,npoints)/),driveFormat,driveDataIn(dInPos,:,:,t)  &
...
!write(*,*) 'update_mod line 679', nfielddrivefile, nfieldfile
...
!        print*, 'drive_tinterp ivar=',ivar,' to interp with inval=',driveDataIn(dInPos,ix,iy,t1:t2)
         CALL tinterp( driveDataPer, &  !Jupp
	              driveDataPer, &   !Jupp
		      1, &              !Jupp
		      driveTimeIndex, &  !Jupp
		      driveDataIn(dInPos,ix,iy,t1:t2),  & !Jupp
                      disAggFlag, &                       !Jupp
		      driveVarInterp(ipos), &             !Jupp
		      driveData(dpos,ix,iy,:) )           !Jupp
...
!ABH Changed to inout
  LOGICAL, INTENT(inout) ::  &!  in scalars
...
!-------------------------------------------------------------------------------
! First find the file.
!  print*,'hard-wiring templateT to false!'
  templateT=.False.
...
!   Again, coded assuming we have restricted dataTimeIndex to 0 or 1.
    ...
    IF ( dataTimeIndex(2)==1 .AND. dataTimeIndex(1)==0 ) then
         CALL timeDate(  &
...
! For now, use an IF clause to deal with optional arguments.
  ...
  ELSE
!Jupp    CALL data_init_vals( dataDateInit,dataPer,dataStepMax,dataStepInit  &
!Jupp                        ,dataTimeInit,dataStep,dataDate,dataTime  &
!Jupp                        ,dataTimeIndex,notNext,callType )
    CALL data_init_vals( DatadateInit,DataPer,datastepmax,DataStepInit & !Jupp
                        ,dataTimeInit,DataStep,dataDate,dataTime  &  !Jupp
                        ,dataTimeIndex,.false.,notNext,callType &    !Jupp
                        ,idummy,0)                                   !Jupp
...
!  Jupp, for tapes:
  use drive_io_vars, only :  &
       drivedata,drivedatain,driveDataStep,drivefile, &
       drivedate,drivefiledate,drivefileonunit, &
       drivefilestep,drivefiletime,driveTime,driveunit, &
       drivefilename,notNextDrive
  use file_utils, only : irecprev
  use forcing, only : sw_down
  use prognostics, only : canht_ft,lai
  use spin_mod, only : spinvalold,ispin
  use veg_io_vars, only : notNextVeg,vegdatain,vegDataStep,vegDate, &
       vegfile,vegfiledate,vegfilename, &
       vegfilestep,vegfiletime,vegTime,vegunit,vegunitfile,&
       vegUpdateStep,vegupdatestepmax
...
!Jupp  INTEGER, OPTIONAL, INTENT(in) ::  &!  optional in scalars
!Jupp    dataUpdateStepInit
!Jupp
!Jupp  INTEGER, OPTIONAL, INTENT(inout) ::  &!  optional inout scalars
!Jupp    dataUpdateStep

  INTEGER, INTENT(in) ::  &!  optional in scalars          !Jupp
    dataUpdateStepInit                                     !Jupp
                                                           !Jupp
  INTEGER, INTENT(inout) ::  &!  optional inout scalars    !Jupp
    dataUpdateStep                                         !Jupp
...
!     if(t.le.t2) then !Jupp
!        print*,'*WARNING* in data_init_vals: attempt to use code hidden from differentiation.' !Jupp
!        stop "stopping in data_init_vals" !Jupp
!     end if !Jupp
!    end do
    
!  ELSE

!Luke reinstated code below


!   Loop over times of data.
    DO t=dataTimeIndex(1),t2

!$TAF STORE datastepmax,sw_down,vegdatastep,vegdatastepmax,vegdate,vegtime,vegupdatestep, &
!$TAF vegupdatestepmax=tape_update, REC= (iloopcount-1)*t2+t
...
!Jupp  IF ( PRESENT(dataUpdateStep) ) dataUpdateStep = dataUpdateStepInit - 1
  IF ( callType=='veg' ) dataUpdateStep = dataUpdateStepInit - 1 !Jupp
...
!   imported arrays with intent(inout)
  ...
  use veg_io_vars, only : vegdatain !Jupp, for tapes
  use fomod, only : iloopcount      !Jupp, for tapes
...
!Jupp  INTEGER, INTENT(in), OPTIONAL ::  &!  optional in SCALARS
  INTEGER, INTENT(in) ::  &!  !Jupp  in SCALARS
...
!   Read veg data.
    CALL veg_read
!Jupp
!write(*,*) 'update_mod 2193 :',iloopcount+1
!$TAF STORE vegupdatestep,vegupdatestepmax = tape_n1, REC = iloopcount + 1
!Jupp
...
!     Establish what field this is.
      ...
      END SELECT
!Jupp
!write(*,*) 'update_mod 2245 :',nvegVarMax*(iloopcount)+ivar
!$TAF STORE varNum = tape_nv, REC = nvegVarMax*(iloopcount)+ivar
!Jupp      IF ( varNum < 0 ) CYCLE  !  nothing to do for this variable
      if (varnum .ge. 0) then !Jupp
...
!         Copy input field (PFT,x) to (PFT,x).
    ...
    ENDDO  !  ivar
!Jupp
!write(*,*) 'update_mod 2272 : ', iloopcount + 1
!$TAF STORE canht_ft,lai = tape_n1, REC = iloopcount + 1
!Jupp
...
!  At start of 1st timestep, any updating is now complete, so write a dump now, if necessary.
   IF ( tstep==1 .AND. vegVaryT .AND. dumpFreq>1 ) CALL dump_io( .TRUE., dumpTypeArg='init' )
...
!Luke uncommented following call to allow prescribed vegetation
...
! 3rd arg (t)=vegDataStep, meaning the one value is for this timestep within the period of validity of input data
  DO ipft=1,npft
    CALL tinterp( 1, &                                       !Jupp
		  ...
		  vegDataStep, &                             !Jupp
		  vegTimeIndex, &                            !Jupp
		  vegDataIn(varNum,1,ipft,t1:t2),  &         !Jupp
                  '0', &                                     !Jupp
		  vegVarInterp(varNum), &                    !Jupp
		  vegData(ipft) )                            !Jupp
  ENDDO
