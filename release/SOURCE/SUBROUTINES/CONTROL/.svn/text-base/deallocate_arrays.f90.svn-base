...
!-------------------------------------------------------------------------------
      IF (ALLOCATED(routeIndex)) THEN                                !Jupp ?bug fix?
         DEALLOCATE( routeindex,stat=ierr); ierrSum=ierrSum+ierr     !Jupp ?bug fix?
      ENDIF                                                          !Jupp ?bug fix?
...
! Jupp code begins
      IF (ALLOCATED(rho_snow)) THEN; DEALLOCATE(RHO_SNOW,stat=ierr); ierrSum=ierrSum+ierr; ENDIF
      IF (ALLOCATED(snow_soil_htf)) THEN; DEALLOCATE(snow_soil_htf,stat=ierr); ierrSum=ierrSum+ierr; ENDIF
! Jupp code ends
...
!Jupp ? not allocated?      IF (ALLOCATED(rrunDiag)) THEN; DEALLOCATE(rrunDiag ,stat=ierr); ierrSum=ierrSum+ierr; ENDIF
...
!-------------------------------------------------------------------------------
    CASE default
      ...
      WRITE(*,*)'ERROR: deallocate_arrays: no code for callType=',callType
      STOP

  END SELECT

  IF ( ierrSum /= 0 ) CALL allocate_Error( 'dealloc',ierrSum,'deallocate_arrays:'//callType )
