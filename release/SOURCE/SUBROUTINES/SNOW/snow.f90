...
USE fomod, only: iloopcount	!Luke
IMPLICIT NONE
...
!EXTERNAL canopysnow,layersnow,relayersnow,snowtherm,snowpack,     &
         !snowgrain,compactsnow !Luke removed

hf_snow_melt(:) = 0.0 ! Jupp initialise variable
lying_snow(:)   = 0.0 ! Jupp initialise variable
...
snomlt_sub_htf(:) = 0.0
snow_melt(:) = 0.0
hf_snow_melt(:)=0.0 !Luke initialise

DO n=1,ntiles
!Luke
!$TAF STORE csnow,ds,ksnow,melt_tile,nsnow,rgrain,rgrain0,rgrainl,rho_snow,rho_snow_grnd,sice,&
!$TAF       sliq,snomlt_sub_htf,snow_can,snow_grnd,snow_soil_htf,snow_tile,snowdepth,tsnow,tsoil1 &
!$TAF       =tape_snow1, REC=(iloopcount-1)*ntiles+n
!Luke
!-----------------------------------------------------------------------
...
!$TAF INCOMPLETE snow_can, melt_tile
...
!-----------------------------------------------------------------------
! DEPENDS ON: snowpack
!$TAF INCOMPLETE ds,melt_tile,sice,sliq, snowdepth,tsnow,snow_soil_htf,rho_snow
...
!$TAF INCOMPLETE nsnow,ds,rgrain,rgrainl,sice,rho_snow_grnd,sliq,tsnow,rho_snow,snowdepth
