...
!logical, external :: all   !Jupp
!integer, external :: count !Jupp
...
! Jupp if (firstcall) down to "else" moved to new subroutine init_spin_check
...
!Jupp    
!$TAF STORE jvar,newval,spinvalold = tape_n_np_spin, & 
!$TAF&      REC = np*nspinvarmax*(iloopcount-1) + np*(ivar-1) + ip
!Jupp
...
!Jupp      spinEnd = .TRUE.
!Jupp      spinUp = .FALSE.
      if (echo) WRITE(*,"(a,i4,a,/,50('#'))")'All points spun up after ',ispin,' cycles.' !Luke
!     Save the number of cycles used.
!Jupp: use all iterations in any case
      if (ispin==nspin) then !Jupp
        spinend = .true.     !Jupp
	spinup = .false.     !Jupp
        nspinFinal = ispin   !Jupp
      end if                 !Jupp
    ELSE
!-------------------------------------------------------------------------------
!     One or more points have not fully spun up yet.
!-------------------------------------------------------------------------------
      IF ( ispin==nspin .AND. echo ) WRITE(*,*)'Model has not fully spun up after max number (=',nspin,') of spin up cycles' !Luke
      jvar = 0
      DO ivar=1,nspinVarMax
        IF ( spinVar(ivar) ) THEN
          jvar = jvar + 1
...
!Jupp          CALL jules_final( 'spinFail' )
...
!Jupp new subroutine follows....
SUBROUTINE init_Spin_Check
...
END SUBROUTINE init_Spin_Check
