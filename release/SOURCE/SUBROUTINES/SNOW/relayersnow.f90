...
DO k=1,tile_pts
!Luke
!$TAF STORE old,rgrain,rgrainl,sice,sliq,tsnow=tape_snow,REC=(iloopcount-1)*tile_pts+k
!Luke
...
!     Loop over the old layers.
    DO old=0,nold(i)
!Luke
!$TAF STORE iznew,newremains=tape_rlsnow1,REC=(iloopcount-1)*tile_pts*(nold(i)+1)+(k-1)*(nold(i)+1)+(old+1)
!Luke
...
!       Loop over new layers with remaining space.
      DO new=izz,nsnow(i)
!Luke
!$TAF STORE newremains,oldremains=tape_rlsnow2,REC= (iloopcount-1)*tile_pts*(nold(i)+1)*nsnow(i) &
!$TAF                                                                +(k-1)*(nold(i)+1)*nsnow(i) &
!$TAF                                                                        +(old+1-1)*nsnow(i) &
!$TAF                                                                                       +new
!!Luke
...
!          EXIT					!Luke commented for adjoint purposes
        END IF
      END DO  !  new layers
