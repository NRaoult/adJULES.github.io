!Jupp
!$TAF SUBROUTINE output_mod::output ACTIVE =
!Jupp
...
!         Output snapshot at current time. Only output time average if >=90% of the expected data exist.
          ...
          IF ( snapProfile(iout) ) THEN
             doSnap = .TRUE.
if(echo)     WRITE(*,"(a,tr1,a,tr1,a,i2,a,i8,i6)")  &
...
!           this time fits into sampling period.
     ...
     'WARNING: outWarnEarly:'  &
        ...
        CASE default
!         Regular output.
if (echo) WRITE(*,*) 'WARNING: outWarnEarly: endSec and no output written because of time mismatch.'
if (echo) WRITE(*,*) 'Output profile #',iout,' date,time at end of timestep=',dateNext,timeNext
if (echo) WRITE(*,*) 'This probably means that the end of a "section" of the run (e.g. spin up)'
if (echo) WRITE(*,*) 'does not fall at a timestep when output would be generated - so there''s no output.'
...
!-------------------------------------------------------------------------------
  INTEGER :: i1,i2,i3,ivar,jvar,kvar,ndy,nhr,nmin,nmon,nsec,ntMain,ntOutTot  &
   ,ntSpin,nxy,nyr,nz,timeTmp,kk,jj,ii  !  work					! Local scalars kk,jj,ii added by Luke/Jupp
...
!--------------------------------------------------------------------------------
  ...
  IF ( outFormat == formatAsc ) THEN
    IF ( .NOT. compressGridCall ) THEN
!      WRITE(outUnit(iout),"(i3,a)") nvarOut(iout),' variables in this file.'       ! Luke/Jupp removed
      WRITE(outUnit(iout),"(a,i3,a)") '# ',nvarOut(iout),' variables in this file.' ! Luke/Jupp added
...
!     NOTE: e.g. monthly output, run ends midmonth, snapshot will be early, so this message is wrong...
      ...
      ENDIF
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    
! code added by Luke/Jupp begins    
    ii = 0 													! ii counts the total number of variables output
    WRITE(outUnit(iout),"(a,i7,a)") '# column',ii+1,' timestep'							! and hence the number of columns needed
    ii = ii+1
    WRITE(outUnit(iout),"(a,i7,a)") '# column',ii+1,' date'							
    ii = ii+1
    WRITE(outUnit(iout),"(a,i7,a)") '# column',ii+1,' time'							
    ii = ii+1
    WRITE(outUnit(iout),"(a,i7,a)") '# column',ii+1,' ispin'							
    ii = ii+1
! code added by Luke/Jupp ends
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      DO ivar=1,nvarOut(iout)
        ...
        SELECT CASE ( varType(jvar) )
          CASE ( 'LA' )
            nz = 1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! beginning of write statements modified by Luke/Jupp
!
! all references to ii and do loops involving jj added by Luke/Jupp
!
!            WRITE(outUnit(iout),"(a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a)")  &			! Luke/Jupp removed
!                         cwork,nz  &									! Luke/Jupp removed
!                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &					! Luke/Jupp removed
!                        ,'(',TRIM(varUnitsList(kvar)),')','defined on land points'			! Luke/Jupp removed
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a)")  &
                         '# column',ii+1,cwork,nz  &
                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &
                        ,'(',TRIM(varUnitsList(kvar)),')','defined on land points'
            ii=ii+1
          CASE ( 'PF' )
!            WRITE(outUnit(iout),"(a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)") &			! Luke/Jupp removed
!                          cwork,npft  &								! Luke/Jupp removed
!                         ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &				! Luke/Jupp removed
!                         ,'(',TRIM(varUnitsList(kvar)),')',', for ',npft,' PFTs'			! Luke/Jupp removed
            do jj=1,npft
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,'_',i2.2,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)") &
                          '# column',ii+1,cwork,npft  &
                         ,ADJUSTL(varName(jvar)),jj,TRIM(varDesc(jvar))  &
...
!                         cwork,nz  &									! Luke/Jupp removed
!                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &					! Luke/Jupp removed
!                        ,'(',TRIM(varUnitsList(kvar)),')',' on river routing grid'			! Luke/Jupp removed
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a)")  &
                         '# column',ii+1,cwork,nz  &
                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &
...
!                         cwork,nz  &									! Luke/Jupp removed
!                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &					! Luke/Jupp removed
!                        ,'(',TRIM(varUnitsList(kvar)),')'  &						! Luke/Jupp removed
!                        ,', for selected point on river routing grid'					! Luke/Jupp removed
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a)")  &
                         '# column',ii+1,cwork,nz  &
                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &
          ...
          CASE ( 'SC' )
!            WRITE(outUnit(iout),"(a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)")   &			! Luke/Jupp removed
!                         cwork,dim_cs1  &								! Luke/Jupp removed
!                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &					! Luke/Jupp removed
!                        ,'(',TRIM(varUnitsList(kvar)),')',', for ',dim_cs1,' soil pool(s)'		! Luke/Jupp removed
            do jj=1,dim_cs1
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,'_',i2.2,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)")   &
                         '# column',ii+1,cwork,dim_cs1  &
                        ,ADJUSTL(varName(jvar)),jj,TRIM(varDesc(jvar))  &
          ...
          CASE ('SI' )
            nz = 1
!            WRITE(outUnit(iout),"(a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1)")  &				! Luke/Jupp removed
!                         cwork,nz  &									! Luke/Jupp removed
!                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &					! Luke/Jupp removed
!                        ,'(',TRIM(varUnitsList(kvar)),')'						! Luke/Jupp removed
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1)")  &
                         '# column',ii+1,cwork,nz  &
                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &
                        ,'(',TRIM(varUnitsList(kvar)),')'
            ii=ii+1
          CASE ( 'SO' )
!            WRITE(outUnit(iout),"(a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)")   &			! Luke/Jupp removed
!                         cwork,nsoil  &								! Luke/Jupp removed
!                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &					! Luke/Jupp removed
!                        ,'(',TRIM(varUnitsList(kvar)),')',', for ',nsoil,' soil layers'		! Luke/Jupp removed
            do jj=1,nsoil
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,'_',i2.2,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)")   &
                         '# column',ii+1,cwork,nsoil  &
                        ,ADJUSTL(varName(jvar)),jj,TRIM(varDesc(jvar))  &
                        ,'(',TRIM(varUnitsList(kvar)),')',', for ',nsoil,' soil layers'
            ii=ii+1
            end do
          CASE ( 'SN' )
!            WRITE(outUnit(iout),"(a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a,i3,a,i3,a)")  &		! Luke/Jupp removed
!                         cwork,ntiles*nsmax  &								! Luke/Jupp removed
!                         ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &				! Luke/Jupp removed
!                         ,'(',TRIM(varUnitsList(kvar)),')',', for ',nsmax  &				! Luke/Jupp removed
!                         ,' snow layers on ',ntiles,' tiles'						! Luke/Jupp removed
            do jj=1,ntiles
              do kk=1,nsmax
              WRITE(outUnit(iout), &
        "(a,i7,1x,a1,tr1,i3,tr1,a,'_',i2.2,'_',i2.2,tr1,a,tr1,a1,a,a1,tr1,a,i3,a,i3,a)")  &
                           '# column',ii+1,cwork,ntiles*nsmax  &
                           ,ADJUSTL(varName(jvar)),jj,kk,TRIM(varDesc(jvar))  &
          ...
          CASE ( 'TI' )
!            WRITE(outUnit(iout),"(a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)")  &			! Luke/Jupp removed
!                         cwork,ntiles  &								! Luke/Jupp removed
!                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &					! Luke/Jupp removed
!                        ,'(',TRIM(varUnitsList(kvar)),')',', for ',ntiles,' tiles'			! Luke/Jupp removed
            do jj=1,ntiles
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,'_',i2.2,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)")  &
                         '# column',ii+1,cwork,ntiles  &
                        ,ADJUSTL(varName(jvar)),jj,TRIM(varDesc(jvar))  &
          ...
          CASE ( 'TY' )
!            WRITE(outUnit(iout),"(a1,tr1,i3,tr1,a,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)" )  &			! Luke/Jupp removed
!                         cwork,ntype  &								! Luke/Jupp removed
!                        ,ADJUSTL(varName(jvar)),TRIM(varDesc(jvar))  &					! Luke/Jupp removed
!                        ,'(',TRIM(varUnitsList(kvar)),')',', for ',ntype,' surface types'		! Luke/Jupp removed
            do jj=1,ntype
            WRITE(outUnit(iout),"(a,i7,1x,a1,tr1,i3,tr1,a,'_',i2.2,tr1,a,tr1,a1,a,a1,tr1,a,i3,a)" )  &
                         '# column',ii+1,cwork,ntype  &
                        ,ADJUSTL(varName(jvar)),jj,TRIM(varDesc(jvar))  &
                        ,'(',TRIM(varUnitsList(kvar)),')',', for ',ntype,' surface types'
            ii=ii+1
            end do
! end of write statements modified by Luke/Jupp
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          CASE default
...
!      WRITE(outUnit(iout),*) undefOut,' missing data value'						! Luke/Jupp removed
    WRITE(outUnit(iout),"(a,e15.3,a)") '# ',undefOut,' missing data value'				! Luke/Jupp added
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! code added by Luke/Jupp begins
    WRITE(outUnit(iout),"(a15)",advance='no') 'timestep' 
    WRITE(outUnit(iout),"(a15)",advance='no') 'date'
    WRITE(outUnit(iout),"(a15)",advance='no') 'time'
    WRITE(outUnit(iout),"(a15)",advance='no') 'spinup'        
    DO ivar=1,nvarOut(iout)
      jvar = varPos(iout,ivar)
      cwork = 'S'  ! snapshot variable
      IF ( taccumVar(jvar) ) cwork = 'A'
      ...
      SELECT CASE ( varType(jvar) )
        CASE ( 'LA' )
          nz = 1
          WRITE(outUnit(iout),"(a20)",advance='no') TRIM(varName(jvar))  
        CASE ( 'PF' )
	  do jj=1,npft
          WRITE(outUnit(iout),"(a15,'_',i2.2)",advance='no') TRIM(varName(jvar)),jj
	  end do
        CASE ( 'RG' )
          nz = 1
          WRITE(outUnit(iout),"(a20)",advance='no') TRIM(varName(jvar)) 
	  ...
	  do jj=1,ntiles
            do kk=1,nsmax
            WRITE(outUnit(iout),"(a15,'_',i2.2,'_',i2.2)",advance='no') TRIM(varName(jvar)),jj,kk
            end do   
	  end do
        CASE ( 'TI' )
	do jj=1,ntiles
...
! Deal with compressGridCall first, as most of the later stuff is not needed in that case.
  IF ( compressGridCall ) THEN
    ...
    dsetLine = fileName
!   A relative path should not be used, since the ctl file will be written in that directory.
!   Remove path and add"^".
    IF ( dsetline(1:1) /= '/' ) dsetLine = '^' // rm_path( dsetLine )
    cper = '-'
...
! A relative path should not appear in dsetline, since the ctl file will be written in that directory.
! Remove path and add"^".
  IF ( dsetline(1:1) /= '/' ) dsetLine = '^' // rm_path( dsetLine )
...
!   A relative path should not be used. Remove path and add"^".
    IF ( outGridNxyame(1:1) /= '/' ) outGridNxyame = '^' // rm_path( outGridNxyame )
...
!       value, and error flagged.
        undefTave = .TRUE.
        outWarnUnder = .TRUE.
if (echo) WRITE(*,"(a,tr1,i7,tr1,a,i2,a,i8,i6)")  &
...
!-----------------------------------------------------------------------
  IF ( outFormat == formatAsc ) THEN
    IF ( .NOT. spinUp ) THEN
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    
! code removed by Luke/Jupp begins
...
! code removed by Luke/Jupp ends
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! code added by Luke/Jupp begins
      WRITE(outunit(iout),*)  ! a blank in order to get a newline
      WRITE(outunit(iout),"(4i15)",advance='no') a_step,outputDate,outputTime,0
    ELSE
      WRITE(outunit(iout),*) ! a blank in order to get a newline
      WRITE(outunit(iout),"(4i15)",advance='no') a_step,outputDate,outputTime,ispin
! code added by Luke/Jupp ends
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ENDIF
  ENDIF
...
!-------------------------------------------------------------------------------
  IF ( haveSnow(iout) .AND. outFormat==formatNc .AND. .NOT.gradsNc ) THEN
   ...
   '########################################################################'
    ia=ia+1; annot(ia) = &
   '#### NB gradsNc=F. This ctl file cannot represent snow layer variables.'
    ia=ia+1; annot(ia) = &
   '########################################################################'
  ENDIF
