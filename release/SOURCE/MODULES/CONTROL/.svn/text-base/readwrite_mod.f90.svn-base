...
!  imported procedures
     readVar2dReal_ncVector_gswp2    !Jupp
...
!      WRITE(unit,*,iostat=ierr) val(:)   !  until get better format!            ! Luke reinstated for dump

!     Get size of input data.
!      np = SIZE( val(:) )							! Luke removed
!     Work out how many lines to be written.					
!      nline = CEILING( REAL(np) / REAL(lineLen) )				! Luke removed
!      j2 = 0									! Luke removed
!      DO i=1,nline								! Luke removed
!        j1 = j2 + 1								! Luke removed
!        j2 = MIN( j1+lineLen-1, np )						! Luke removed
!        WRITE(unit,*,iostat=ierr) val(j1:j2)					! Luke removed
        WRITE(unit,"(e15.5)",advance='no',iostat=ierr) val(:)   		! Luke/Jupp added - remove for dump
!       If error, call error routine, indicating a stop.
