!Jupp
!$TAF MODULE misc_utils SUBROUTINE getfields INPUT  = 1, 2, 3, 4, 5, 6,    8, 9   
!$TAF MODULE misc_utils SUBROUTINE getfields OUTPUT =                   7,       10, 11, 12        
!$TAF MODULE misc_utils SUBROUTINE getfields ACTIVE = 
!$TAF MODULE misc_utils SUBROUTINE getfields DEPEND = 
!Jupp
...
!Jupp         IF ( PRESENT(nfield) .AND. varPos(i)>nfield ) THEN
      IF ( PRESENT(nfield) ) then                                  !Jupp
         if ( varPos(i)>nfield ) THEN                              !Jupp
          errFlag = .TRUE.       
...
!   Search for a %.
    ...
    i2 = ipos !Jupp: to fix in TAF + LEN(templString) - 1
...
!-------------------------------------------------------------------------------
   ...
   if (echo) write(*,*) 'inside check_template'
...
!Jupp: to fix in TAF     i2 = MIN( ipos+LEN(templString)-1,ILEN )
    i2 = 1!Jupp
...
!   Set index for next loop.
!Jupp    i1 = ipos + LEN(templString)
    i1 = ipos !Jupp: to fix in TAF + LEN(templString)
